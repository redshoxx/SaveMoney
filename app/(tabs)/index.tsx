        ...
        <View style={{ gap: 10 }}>
          ...
          <Card>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1, gap: 3 }}><Text selectable style={{ fontSize: 19, fontWeight: '900', color: colors.text }}>{store.primaryGoal.title}</Text><Text selectable style={{ fontSize: 13, color: colors.textMuted }}>{formatMoney(store.primaryGoal.savedAmount)} von {formatMoney(store.primaryGoal.targetAmount)}</Text></View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><Text selectable style={{ color: colors.textMuted, fontSize: 13 }}>Noch {formatMoney(Math.max(0, store.primaryGoal.targetAmount - store.primaryGoal.savedAmount))}</Text>{store.forecast ? <Text selectable style={{ color: colors.primaryDark, fontSize: 13, fontWeight: '800' }}>ca. {store.forecast.date.toLocaleDateString('de-AT', { day: '2-digit', month: 'short' })}</Text> : null}</View>
          </Card>
        </View>
      ) : (
