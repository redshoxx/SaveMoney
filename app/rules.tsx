...
export default function RulesScreen() {
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: 70, gap: 20 }}>
      <View style={{ gap: 5 }}><Text selectable style={{ color: colors.text, fontSize: 25, fontWeight: '900' }}>Sparregeln</Text><Text selectable style={{ color: colors.textMuted, fontSize: 14, lineHeight: 20 }}>SparFlow zieht kein echtes Geld ein. Fällige Regeln erscheinen in der App und lassen sich mit einem Tap als Sparbuchung bestätigen.</Text></View>
      ...
      <Card>
        ...
        <View style={{ gap: 8 }}><Text style={{ color: colors.text, fontWeight: '800' }}>Ziel</Text>{store.goals.map((goal) => <Pressable key={goal.id} onPress={() => setGoalId(goal.id)} style={({ pressed }) => ({ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 11, borderRadius: 14, backgroundColor: goalId === goal.id ? colors.primarySoft : colors.surfaceMuted, opacity: pressed ? 0.7 : 1 })}><Symbol name={goal.icon} size={17} color={goal.color} /><Text style={{ flex: 1, color: colors.text, fontWeight: '800' }}>{goal.title}</Text>{goalId === goal.id ? <Symbol name="checkmark.circle.fill" size={18} color={colors.primary} /> : null}</Pressable>)}</View>
      ...
      {store.savingRules.length === 0 ? <EmptyState icon="clock.badge.checkmark" title="Noch keine Sparregel" body="Erstelle eine wiederkehrende Erinnerung für kleine Sparbeträge." /> : (
        <View style={{ gap: 11 }}>
          {store.savingRules.map((rule) => {
            ...
            return (
              <Card key={rule.id}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><IconBubble icon="clock.fill" /><View style={{ flex: 1, gap: 3 }}><Text selectable style={{ color: colors.text, fontWeight: '900', fontSize: 16 }}>{rule.title}</Text><Text selectable style={{ color: colors.textMuted, fontSize: 12.5 }}>{formatMoney(rule.amount)} · {rule.frequency === 'daily' ? 'täglich' : rule.frequency === 'weekly' ? 'jeden Freitag' : 'am 1. des Monats'} · {goal?.title ?? 'Ziel'}</Text></View><Switch value={rule.enabled} onValueChange={(enabled) => void store.toggleRule(rule.id, enabled)} /></View>
                {due ? <Pressable onPress={() => void store.applyRule(rule.id)} style={({ pressed }) => ({ minHeight: 45, borderRadius: 14, backgroundColor: colors.primarySoft, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, opacity: pressed ? 0.7 : 1 })}><Symbol name="checkmark.circle.fill" size={17} color={colors.primaryDark} /><Text style={{ color: colors.primaryDark, fontWeight: '900' }}>Heute {formatMoney(rule.amount)} sparen</Text></Pressable> : <Text selectable style={{ color: colors.textMuted, fontSize: 12 }}>Aktuell nicht fällig.</Text>}
